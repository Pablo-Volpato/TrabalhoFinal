...
const AtualizarTiposDeDispositivos = () => {
  ...
  const cadastrarTiposDeDispositivos = async (event: any) => {
    ...
    try {
      setLoading(true);
      await axios.put(
        `/tiposDeDispositivos/${id}?idUsuario${usuarioId}`,
        dados
      );
  ...
  const obterTiposDeDispositivosPorId = async () => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    obterTiposDeDispositivosPorId();
  }, [id]);

  const deletarTipoDeDispositivo = async () => {
    try {
      setLoading(true);
      ...
      <Col>
        ...
        <div style={{ display: "flex", justifyContent: "row" }}>
          <div className="card">
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <div className="titulo-container">
                ...
                <h6 className="subtitulo">
                  {"Cadastro > Tipos de Dispositivos > Atualizar"}
                </h6>
              </div>
              <DeleteModal registro={nome} onPress={deletarTipoDeDispositivo}>
                <i
            ...
            <Dialog
              header="Tipos De Dispositivo foi Atualizado com sucesso!"
              footer={
                <>
                  <Button
                    label="OK"
                    onClick={() => history.push("/tipos-de-dispositivos/1")}
                  />
                </>
              }
            ...
            {loading ? (
              ...
              <Form
                noValidate
                validated={validated}
                onSubmit={cadastrarTiposDeDispositivos}
              >
                <Form.Row>
                  <Form.Group as={Col} sm={6}>
                    ...
                    <Form.Control
                      ...
                      name="Descrição"
                      type="text"
                      title="Preencha com a descrição"
                      maxLength={256}
                    />
                  </Form.Group>
                </Form.Row>
...
export default AtualizarTiposDeDispositivos;
